...
mod tests {
    ...
    fn parse_basic() {
        let landmark = Landmark::parse(
            r#"{"hand":"Right","points":[{"x":100,"y":200},{"x":10,"y":20}]}"#,
        )
    ...
    fn parse_left_hand() {
        let landmark =
            Landmark::parse(r#"{"hand":"Left","points":[{"x":-5,"y":-10}]}"#)
                .expect("valid landmark should parse");
    ...
    fn parse_rejects_unknown_hand() {
        assert!(
            Landmark::parse(r#"{"hand":"Middle","points":[{"x":1,"y":2}]}"#).is_none()
        );
    }
    ...
    fn parse_rejects_malformed_point() {
        assert!(
            Landmark::parse(r#"{"hand":"Right","points":[{"x":1}]}"#).is_none()
        );
        assert!(Landmark::parse(r#"{"hand":"Right","points":"1,2"}"#).is_none());
        assert!(
            Landmark::parse(r#"{"hand":"Right","points":[{"x":40000,"y":0}]}"#)
                .is_none()
        );
    }
}
